...
```{r comparacion-ic-visual, echo=TRUE, fig.height=4.5}
ggplot(comparacion_IC, aes(x = reorder(Modelo, AICc), y = AICc, 
                           fill = Modelo == "ARIMA(1,1,1) + drift")) +
  geom_col(color = "white", linewidth = 0.8) +
  geom_text(aes(label = round(AICc, 1)), vjust = -0.5, fontface = "bold") +
  scale_fill_manual(values = c("TRUE" = qqq_pal$positive, 
                               "FALSE" = qqq_pal$secondary), guide = "none") +
  labs(title = "Comparación de Criterios IC: AICc por Modelo",
       subtitle = "Menor AICc = Mejor balance ajuste-complejidad",
       x = "Modelo", y = "AICc",
       caption = "Verde: Modelo seleccionado") +
  theme_QQQ() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

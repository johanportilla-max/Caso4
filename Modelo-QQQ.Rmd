...
$$H_0: \text{Serie tiene raíz unitaria (no-estacionaria)}$$
...
El estadístico ADF de -8.6831 es altamente negativo, muy inferior al valor crítico de -3.43. Con p-valor de 0.01 < 0.05, se rechaza $H_0$ con confianza
...
El test de Ljung-Box proporciona contraste formal de la hipótesis nula de que residuos constituyen ruido blanco versus hipótesis alternativa de que existe autocorrelación residual significativa. La estadística de Ljung-Box, modificación del test de Box-Pierce de Ljung & Box (1978), se define como:
$$
Q^* = n(n+2) \sum_{k=1}^{m} \frac{\hat{\rho}_k^{\,2}}{n - k}
$$
donde $n$ es el número de residuos, $m$ es el número de rezagos evaluados, y $ρ̂ₖ $ es la autocorrelación muestral en rezago k. Bajo hipótesis nula de independencia, $Q^*$ sigue aproximadamente distribución chi-cuadrado con $m$ grados de libertad (Box & Jenkins, 1976; Hamilton, 1994)
...
El modelo ARIMA(1,1,1) con drift, habiendo superado todas las pruebas de diagnóstico de residuos, procede ahora a la etapa de pronóstico: generación de predicciones puntuales de precios futuros para el QQQ junto con intervalos de confianza que cuantifican la incertidumbre inherente. La práctica de pronóstico en series de tiempo requiere distinción crítica entre dos enfoques complementarios: pronóstico ex-ante (predicción de valores verdaderamente desconocidos en horizonte futuro) versus pronóstico ex-post (predicción de valores conocidos pero retenidos como conjunto de prueba para evaluación). El análisis que sigue utiliza ambos enfoques: primero se generan pronósticos ex-post sobre el conjunto de prueba (octubre 2025, 10 observaciones retenidas) para evaluar precisión del modelo, y luego se presentan pronósticos ex-ante sobre horizonte futuro inmediato.
...
Lo notable es la simetría aproximada de errores: errores positivos en días 1-7 son aproximadamente balanceados por errores negativos en días 8-10. Esto indica que el modelo no tiene sesgo sistemático direccional—es aproximadamente insesgado aunque comete errores individuales típicos de predicción financiera (Tsay, 2010; Hamilton, 1994).

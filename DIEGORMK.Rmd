---
title: "Aplicación de Modelos ARIMA y Análisis de Series de Tiempo para la Predicción de Precios de Cierre del ETF QQQ (2022-2025)"
subtitle: "Evaluación de la volatilidad, tendencias del sector tecnológico y proyecciones estocásticas de corto plazo"
author: "Portilla Aguirre Johan Camilo (2422468), Barros Rayo Alejandro (2415837), Muñoz Portela Diego Fernando (2415620) "
...
/* ========== NUEVO: ESTILO PARA EL BOTÓN INDIVIDUAL DE CADA CHUNK ========== */
.chunk-code-toggle {
  ...
  font-size: 0.85rem;
  font-weight: 600;
  margin-bottom: 5px; /* Espacio entre botón y código */
...
/* Ajuste para que el bloque de código (pre) aparezca pegado al botón */
pre {
  margin-top: 0 !important;
  transition: all 0.3s ease;
}
/* ========== CENTRADO UNIVERSAL DE GRÁFICOS E IMÁGENES ========== */

/* 1. Centra imágenes estáticas (tanto Markdown como HTML <img>) */
img {
display: block;
 margin-left: auto;
 margin-right: auto;
...
/* 2. Centra contenedores de figuras generados por R (gráficos estáticos) */
.r-figure {
 display: block;
 margin-left: auto;
 margin-right: auto;
 max-width: 900px; /* O el ancho máximo que desees */
}
...
table, .table, table.kable_wrapper {
    ...
    max-width: 900px; /* Mismo ancho máximo que tus gráficos */
    width: auto !important; /* Evita que la tabla se estire al 100% si es pequeña */
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // --- 2. NUEVA LÓGICA: BOTONES INDIVIDUALES PARA CADA CÓDIGO ---
  
  // Seleccionamos todos los bloques de código generados por R
  ...
  codeBlocks.forEach(block => {
    // 1. Ocultar el código por defecto
    block.style.display = 'none';

    // 2. Crear el botón
    const btn = document.createElement('button');
    btn.className = 'chunk-code-toggle';
    btn.innerHTML = '<span>&#9654;</span> Ver Código'; // Icono de play
    
    // 3. Insertar el botón ANTES del bloque de código
    ...
    btn.addEventListener('click', function() {
      if (block.style.display === 'none') {
        ...
        btn.innerHTML = '<span>&#9660;</span> Ocultar Código';
        btn.classList.add('active');
      } else {
        // Ocultar
        block.style.display = 'none';
        btn.innerHTML = '<span>&#9654;</span> Ver Código';
  ...
  // Eliminé el botón global anterior para que no tengas conflicto, 
...
Las estadísticas descriptivas del nivel de precios refuerzan la lectura estructural del comportamiento del QQQ en el periodo 2010–2025. A diferencia de los rendimientos —que fluctúan alrededor de cero— los niveles de precio capturan la magnitud absoluta del crecimiento del índice y el sesgo temporal que caracteriza su trayectoria ascendente.

El promedio de la muestra (202.67 USD) se sitúa muy por encima de la mediana (152.08 USD), una diferencia que revela una asimetría positiva moderada (Skewness ≈ 0.92). Esto implica que la distribución está desplazada hacia valores altos, lo cual es consistente con la aceleración del precio en los años recientes. En otras palabras, aunque el QQQ pasó gran parte del periodo operando en niveles intermedios, la fuerte expansión desde 2020 —particularmente entre 2023 y 2025— “empuja” el promedio hacia arriba. Desde el punto de vista financiero, esta asimetría refleja que una porción significativa de la creación de valor se concentra en el tramo final de la serie, en línea con el auge tecnológico vinculado a inteligencia artificial.

El recorrido completo del precio, que va desde un mínimo de 37.14 USD hasta un máximo de 635.77 USD, muestra una amplitud cercana a 600 USD, coherente con una tendencia secular de gran magnitud. Esta amplitud se expresa también en la desviación estándar (150.65 USD), que más que capturar riesgo “instantáneo”, sirve aquí como medida del tamaño de la tendencia: un activo que multiplica varias veces su valor en quince años exhibe, por construcción, una dispersión elevada en sus niveles observados.

La curtosis de 2.78 —ligeramente inferior al valor de referencia normal (=3)— indica una distribución mesocúrtica, es decir, sin una concentración excesiva en los extremos. Sin embargo, esto no contradice el comportamiento observado: al tratarse de precios y no de retornos, es común que las colas sean menos pronunciadas, ya que la tendencia de largo plazo suaviza parte de la variabilidad relativa. Lo relevante aquí es que la distribución no se ajusta a un patrón estrictamente simétrico ni estacionario, lo cual coincide con la naturaleza del QQQ como activo fuertemente direccionado por ciclos tecnológicos.

En conjunto, estas métricas reafirman que la estructura estadística del precio del QQQ está dominada por una tendencia alcista persistente, episodios de aceleración reciente y una asimetría que refleja cómo la mayor parte de la apreciación del valor se concentra en la última fase del periodo analizado.
...
Las estadísticas básicas del nivel de precios confirman una estructura marcada por una tendencia ascendente persistente y un aumento significativo de los valores en los últimos años. El promedio de la muestra (202.67 USD) se ubica claramente por encima de la mediana (152.08 USD), lo que evidencia una asimetría positiva moderada. Esta diferencia indica que los registros más recientes —donde el QQQ alcanzó niveles históricamente altos— desplazan la distribución hacia la parte superior del rango, reflejando que buena parte de la apreciación del valor se concentra en la etapa final del periodo analizado.

El mínimo (37.14 USD) y el máximo (635.77 USD) delimitan un rango amplio de casi 600 USD, coherente con un activo que ha multiplicado varias veces su precio desde 2010. Esta amplitud también se manifiesta en la desviación estándar (150.65 USD) y en la varianza asociada, que más que representar “riesgo” en sentido estricto, describen la magnitud de la tendencia de largo plazo: un activo con una trayectoria fuertemente alcista necesariamente presenta una dispersión elevada en sus niveles históricos.

La asimetría de 0.92 confirma el desplazamiento de la distribución hacia valores altos, en línea con la aceleración reciente del índice. Por su parte, la curtosis de 2.78 —ligeramente inferior al valor normal— sugiere una distribución mesocúrtica, sin una concentración extrema en las colas pero tampoco completamente simétrica o estacionaria, algo típico en series de precios que siguen una tendencia estructural fuerte.

En conjunto, estas métricas describen una serie dominada por la apreciación sostenida del QQQ, con una distribución inclinada hacia los niveles altos y una dispersión que refleja la magnitud del crecimiento acumulado a lo largo del periodo.
...
En síntesis, la volatilidad anualizada del 20.80% no solo caracteriza el riesgo promedio del QQQ, sino que encaja con la narrativa general: un activo con una tendencia estructural potente, pero atravesado por episodios de estrés que amplifican temporalmente sus oscilaciones.
...
En conjunto, la forma de la distribución no solo plasma el comportamiento estadístico de los rendimientos, sino que funciona como una evidencia visual directa de la propia identidad del QQQ:
un vehículo altamente concentrado, dominado por factores de crecimiento, sensible a liquidez y a ciclos tecnológicos, y estructuralmente propenso a movimientos extremos más frecuentes que los esperados bajo supuestos gaussianos.
...
```{r rendimientosaño, echo=TRUE, eval=TRUE, warning=FALSE, message=FALSE}
ret_anual_df <- data.frame(
...
## 4 Estacionalidad Mensual del QQQ (2010–2025)
...
El modelo genera pronósticos con error promedio del 0.94%, lo cual es bajo. Pero en el período de prueba, el intervalo de confianza al 95% solo cubrió el 84.44% de los valores reales. Esto significa que el modelo 
predijo con demasiada confianza, la volatilidad real fue mayor a la que el modelo estimó.
...
En conclusión, el modelo ARIMA(1,1,1) con drift es la especificación correcta para el QQQ. 
Estadísticamente funciona, los residuos son ruido blanco, la diferenciación es la apropiada, el modelo supera a alternativas simples. El problema es que esto no se traduce automáticamente en predicciones prácticas útiles. La razón es que el QQQ opera en un mercado eficiente, donde miles de instituciones procesan información constantemente. Cualquier patrón que sea predecible es arbitrajado antes de que pueda explotarse. El modelo lo identifica correctamente, la estructura existe pero es débil.

Si se construye un modelo más sofisticado y no supera notablemente a ARIMA simple, se está probablemente sobreajustando. También permite cuantificar lo que 
el mercado espera, 25% de retorno anualizado durante este período. Y permite monitorear cuándo la estructura cambia, si los residuos comienzan a mostrar autocorrelación nuevamente, es señal de que algo cambió en el mercado. Para investigación futura, agregar variables exógenas (tasas, volatilidad), modelar volatilidad condicional con GARCH, y detectar cambios de régimen son los caminos claros.
...
">
  ...
  <div style="margin-top: 20px; padding-top: 20px; border-top: 1px solid rgba(124, 58, 237, 0.3);">
    <strong style="background: linear-gradient(135deg, #06b6d4, #84cc16); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">ESTUDIANTE:</strong> Portilla Aguirre Johan Camilo (2422468), Barros Rayo Alejandro (2415837), Muñoz Portela Diego Fernando (2415620)<br>

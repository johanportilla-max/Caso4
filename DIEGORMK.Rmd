...
document.addEventListener('DOMContentLoaded', function() {
  const SCROLL_OFFSET = 100; // Offset para dejar espacio al encabezado al hacer scroll
  
  // 1. CREACIÓN DEL TOC "CONTENIDO" (#custom-toc)
  ...
  document.querySelectorAll('h1, h2').forEach(header => {
    // Paso Crítico 1: Asegurar que el header tiene un ID limpio
    if (!header.id) {
      header.id = header.textContent.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, '');
    }

    // Crear el enlace en la TOC
 ...
 // 2. SCROLL SUAVE AL HACER CLIC (Se aplica a los enlaces que acabamos de crear)
document.querySelectorAll('#custom-toc a[href^="#"]').forEach(anchor => {
 anchor.addEventListener('click', function (e) {
  ...
  if (target) {
  window.scrollTo({
   top: target.offsetTop - SCROLL_OFFSET, // <-- Se desplaza 100px menos del target
    behavior: 'smooth'
   });
  }
 });
  ...
  // 3. MARCAR ENLACE ACTIVO AL HACER SCROLL (Para el resaltado)
  ...
  // 4. OTROS ELEMENTOS Y EVENTOS
  
  // Botón de alternar código
  ...
  // Lógica de rotación del botón de la universidad

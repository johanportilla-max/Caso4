...
```{r residuos-qq, echo=TRUE, fig.height=12}
residuos_std <- scale(residuals(ModeloQA))
df_qq <- data.frame(
  residuos = residuos_std,
  id = 1:length(residuos_std)
)

p_qq <- ggplot(df_qq, aes(sample = residuos, text = paste0(
  "Observación: ", id, "<br>",
  "Residuo Std: ", round(residuos, 3), "<br>",
  "Desv. del ajuste: ", round(residuos, 3)
))) +
  stat_qq(color = qqq_pal$primary, size = 2.8, alpha = 0.75) +
  stat_qq_line(color = qqq_pal$negative, linewidth = 1.2) +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = Inf,
           fill = NA, color = qqq_pal$secondary, linewidth = 0.3, alpha = 0.05) +
  ...
  theme(
    ...
    grid.major = element_line(color = "#e8eef5", linewidth = 0.3)
  )

plotly::ggplotly(p_qq, tooltip = "text") %>%
  plotly::layout(
    font = list(family = "Arial, sans-serif", size = 11, color = qqq_pal$text_dark),
    title = list(font = list(size = 14, color = qqq_pal$primary)),
    plot_bgcolor = "#f8f9fa",
    paper_bgcolor = "white",
    xaxis = list(
      showgrid = TRUE,
...
```{r residuos-tabla, echo=TRUE, fig.height=12}
